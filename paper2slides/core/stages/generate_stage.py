...
async def run_generate_stage(base_dir: Path, config_dir: Path, config: Dict) -> Dict:
    ...
    images = generator.generate(
        plan, gen_input, max_workers=max_workers, save_callback=save_image_callback
    )
    ...
    # Generate PPTX for slides
    if output_type == "slides" and len(images) > 0:
        try:
            from paper2slides.generator.pptx_generator import create_pptx_from_images

            image_files = []
            for img in images:
                ext = ext_map.get(img.mime_type, ".png")
                fp = output_subdir / f"{img.section_id}{ext}"
                if fp.exists():
                    image_files.append(fp)
            if image_files:
                create_pptx_from_images(image_files, output_subdir / "slides.pptx")
